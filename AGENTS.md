...
**테스트 가능한 코드는 실패하는 테스트 없이 한 줄도 수정/추가하지 않는다.**
모든 로직 변경은 `red → green → refactor → green` 사이클을 따른다.
예외 — 단순 SwiftUI 뷰·테스트 불가능한 I/O 글루는 RED를 생략하고 빌드·실행으로 검증한다.
...
Xcode 26.5 / Swift 6.0 / iOS 26.1 / Bundle ID `azhy.cue`.
