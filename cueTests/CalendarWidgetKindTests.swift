...
struct CalendarWidgetKindTests {
    ...
    /// 등록된 캘린더 위젯 4종이 빠짐없이 들어 있어야 한다 — 하나라도 빠지면 그 위젯만
    /// 조용히 낡은 그림을 들고 있게 되고, 목록을 보는 쪽에서는 알 방법이 없다.
    @Test func coversEveryCalendarWidget() {
        #expect(CalendarWidgetKind.all.count == 4)
    ...
    @Test func includesWidgetWithoutRangeKind() {
        let shiftable = Set(WidgetRangeKind.allCases.map(\.widgetKind))
        let fixedOnly = CalendarWidgetKind.all.filter { !shiftable.contains($0) }

        #expect(fixedOnly == ["azhy.cue.widget.monthFixed"])
    }
