...
struct ReminderView: View {
    ...
    // 새 항목 저장이 진행 중인지 — 입력칸은 add가 낙관 삽입을 마친 **뒤에** 비우므로
    // (텍스트가 사라지는 공백 구간 제거), 그 사이 return/포커스 해제로 같은 텍스트가
    // 두 번 제출되는 걸 막는다.
    @State private var pendingNewSubmission = false
    ...
    private func submitNewReminder() {
        guard !pendingNewSubmission else { return }
